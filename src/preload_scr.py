...
class PreLoadScr:
    def __init__(self, session: Session, objective: types.Items) -> None:
        ...
        self.step = 0
        self.font = pg_font.Font(FONT[0], FONT[1] * 2)
    def update(self, session: Session, display):
        ...
        if self.bg_offset_y > -165:
            ...
            for key in session.level_data[self.objective.name]["Items"]:
                ...
                disp = self.font.render(str(required_items_count), True, (0, 104, 170))
                ...
                frame = pg_draw.rect(display, (208, 237, 255), pg_rect.Rect(disp_rect.x + 30, disp_rect.y, session.collectibles[key.upper()].get_width() - 60, disp_rect.height), 0, 15)
